...
router.post("/deleteSerial", async (req, res) => {
...
router.get("/viewSerials", async (req, res) => {
...
/* activate serial */
router.post("/activation", uploadWarranty.single("image"), async (req, res) => {
  const {
    name,
    phoneNumber,
    birthdate,
...
router.get("/activatedWarrantys", async (req, res) => {
  const authHeader = req.headers["authorization"];
  if (!authHeader || !authHeader.startsWith("Bearer ")) {
    return res.status(401).send("No token provided or invalid format");
  }
...
router.delete("/activation/:serialNumber", async (req, res) => {
...
router.delete("/activations", async (req, res) => {
  try {
...
router.post("/offerCheck", async (req, res) => {
  const { Id } = req.body;
  try {
...
router.post("/offerUnCheck", async (req, res) => {
  const { Id } = req.body;
  try {
    const newOffer = await Offer.findOne({ Id });
...
router.delete("/deleteOffers", async (req, res) => {
  try {
...
router.get("/bookForms", async (req, res) => {
  const authHeader = req.headers["authorization"];
  if (!authHeader || !authHeader.startsWith("Bearer ")) {
    return res.status(401).send("No token provided or invalid format");
  }
...
router.delete("/bookForm/:id", async (req, res) => {
  const { id } = req.params;
